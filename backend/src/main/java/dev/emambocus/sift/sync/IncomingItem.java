...
public record IncomingItem(
		 ...
		 * fragment differs, which leaves the thread id as the adapter's to supply.
		 */
		String conversationId,
		Instant sourceCreatedAt,
		/**
		 * When this last moved. The feed orders and timestamps by it: for anything long-lived, a
		 * merge request awaiting review say, this is the latest activity and not the creation date.
		 */
		Instant activityAt,
		/**
		 * True when the source itself already considers this seen. Applied only where the row is
		 * first inserted; Sift's own read state owns it from then on. Without it, connecting a
		 ...
		 * the search haystack. A message is found by what was attached to it as well as by what it
		 * says.
		 */
		...
		String searchExtra,
		/**
		 * True when this is state the source keeps reporting: its disappearance means it is done.
