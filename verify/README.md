...
The callback hands the browser back before the read it starts has stored anything. A suite has to
...
GitLab stub is on 7788 and the Google stub on 7790, which do not collide; the backend and the
database do. Two at once, or one launched in the background, produces a wall of failures that
...
app on 7777. Nothing here can touch an instance you are using.
...
Beware: **any** later `npm install` prunes unsaved packages. Re-check that it still resolves before
assuming the suite can run.

## The stand-in GitLab

`fake-gitlab.py` serves `/api/v4/user`, `todos`, `merge_requests`, `issues`, a resource's
`discussions` and the caller's own `events`, from JSON fixtures it re-reads on every request. A test
can therefore change what the instance returns between sweeps. `make-todos.py` and `make-mrs.py` write
...
Unconfigured routes answer with an empty list rather than 404. A suite that does not care about
...
`batchModify` really changes the mailbox and records the change: a push from Sift comes back on the
next sweep and the two agree. A stub that took the call and did nothing would make a correct
...
an `authorization_code` grant with no `code_verifier`. It accepts only its newest access token: a
...
- **A stub must model the provider, not a stricter provider.** Rotating the refresh token on every
  ...
  refresh token is long-lived. This one keeps the first and only mints another on a fresh consent.
- **A fixture stamped relative to "now" rewrites the past.** `make-mail.py` takes a base time and
  `verify-gmail.sh` passes the same one to every invocation. Rewriting the mailbox between sweeps
  then adds messages without moving the ones already read past the watermark.
...
search of the feed query. It needs no shell script and no free ports, which makes it the one to
reach for first. The suites here cover what it cannot: real HTTP, a real browser, and the
packaged container.
